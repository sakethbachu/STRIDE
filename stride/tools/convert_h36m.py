...
def save_clips_bedlam(subset_name, root_path, train_data):
    len_train = len(train_data)
    save_path = os.path.join(root_path, subset_name)
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    for i in tqdm(range(len_train)):
        data_input = train_data[i]
...
dataset = datareader.read_bedlam_psuedo_labels(stride=243)
...
root_path = "data/motion3d/consistent_pose/H36M_bedlam"
if not os.path.exists(root_path):
    os.makedirs(root_path)

# save_clips_bedlam("train", root_path, train_data)
# save_clips("test", root_path, test_data, test_labels)
