...
def compute_similarity_transform_batch_pitchyawroll(S1, S2):
    ...
    for i in range(S1.shape[0]):
        S1_hat[i], S1_pitch[i], S1_yaw[i], S1_roll[i] = (
            compute_similarity_transform_pitchyawroll(S1[i], S2[i])
        )
    return S1_hat, S1_pitch, S1_yaw, S1_roll
...
def reconstruction_error_pitchyawroll(S1, S2):
    ...
    pred_hat, pred_pitch, pred_yaw, pred_roll = (
        compute_similarity_transform_batch_pitchyawroll(S1, S2)
    )
