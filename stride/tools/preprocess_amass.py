...
with open("data/AMASS/clip_list.csv", "w") as f:
    ...
    for i, bdata in tqdm(enumerate(motion_data)):
        if i % 200 == 0:
            print(i, "seqs done.")
        ...
        bm_fname = osp.join(
            "data/AMASS/body_models/smplh/{}/model.npz".format(subject_gender)
        )
        dmpl_fname = osp.join(
            "data/AMASS/body_models/dmpls/{}/model.npz".format(subject_gender)
        )
        ...
        for sid in range(num_slice + 1):
            ...
            body_parms = {
                "root_orient": torch.Tensor(bdata["poses"][start:end, :3]).to(
                    comp_device
                ),  # controls the global root orientation
                "pose_body": torch.Tensor(bdata["poses"][start:end, 3:66]).to(
                    comp_device
                ),  # controls the body
                "pose_hand": torch.Tensor(bdata["poses"][start:end, 66:]).to(
                    comp_device
                ),  # controls the finger articulation
                "trans": torch.Tensor(bdata["trans"][start:end]).to(
                    comp_device
                ...
                "dmpls": torch.Tensor(bdata["dmpls"][start:end, :num_dmpls]).to(
                    comp_device
