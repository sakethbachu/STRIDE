...
def render_and_save(
    ...
    if motion.shape[1] == 2 or with_conf:
        ...
        motion2img_3d(
            motion_world, img_name, save_path=save_path, keep_imgs=keep_imgs, fps=fps
        )


def render_and_save_new(
    motion_input,
    img_name,
    loss,
    save_path,
    ...
    motion1 = copy.deepcopy(motion_input[0])
    # motion2 = copy.deepcopy(motion_input[1])
    # motion3 = copy.deepcopy(motion_input[2])
    if motion1.shape[-1] == 2 or motion1.shape[-1] == 3:
        motion1 = np.transpose(motion1, (1, 2, 0))  # (T,17,D) -> (17,D,T)
        # motion2 = np.transpose(motion2, (1,2,0))
        # motion3 = np.transpose(motion3, (1,2,0))
    # if motion.shape[1]==2 or with_conf:
    #     colors = hex2rgb(color)
    #     if not with_conf:
    #         J, D, T = motion.shape
    ...
    # motion_world3 = pixel2world_vis_motion(motion3, dim=3)
    motion_world = [motion_world1]
    # motion2video_3d(motion_world, save_path=save_path, keep_imgs=keep_imgs, fps=fps)
    motion2img_3d(
        motion_world, img_name, loss, save_path=save_path, keep_imgs=keep_imgs, fps=fps
    )
...
def vis_data_batch(
    data_input, data_label, n_render=10, save_path="doodle/vis_train_data/"
):
    """
...
def joints2image(
    ...
    if nr_joints == 49:  # full joints(49): basic(15) + eyes(2) + toes(2) + hands(30)
        ...
        colors_joints = (
            [M, M, L, L, L, R, R, R, M, L, L, L, L, R, R, R, R, R, L]
            + [L] * 15
            + [R] * 15
        )

        colors_limbs = [M, L, R, M, L, L, R, R, L, R, L, L, L, R, R, R, R, R]
        ...
        colors_joints = [M, M, L, L, L, R, R, R, M, L, L, L, R, R, R]

        colors_limbs = [M, L, R, M, L, L, R, R, L, R, L, L, R, R]
...
def motion2img_3d(motion, img_name, loss, save_path, fps=25, keep_imgs=False):
    #     motion: (17,3,N)
    # videowriter = imageio.get_writer(save_path, fps=fps)
    motion1 = motion[0]
    # motion2 = motion[1]
    ...
    for f in tqdm(range(vlen)):
        fname = img_name[f][0]
        fname = "/".join(fname.split("/")[-3:]).replace("/", "_")
        final_save_path = os.path.join(save_path, fname)
        # j3d = motion[:,:,f]
        # fr_mpjpe = mpjpe[f]*1000
        # fr_pampjpe = pampjpe[f]*1000
        # fr_mpjpe_pf = mpjpe_pf[f]*1000
        # fr_pampjpe_pf = pampjpe_pf[f]*1000

        j3d1 = motion1[:, :, f]
        # j3d2 = motion2[:,:,f]
        # j3d3 = motion3[:,:,f]
        # hip1 = (j3d1[2]+j3d1[3])/2; j3d1_hip = np.concatenate((j3d1,hip1.reshape(1,3)), axis=0)
        # hip2 = (j3d2[2]+j3d2[3])/2; j3d2_hip = np.concatenate((j3d2,hip2.reshape(1,3)), axis=0)
        # hip3 = (j3d3[2]+j3d3[3])/2; j3d3_hip = np.concatenate((j3d3,hip3.reshape(1,3)), axis=0)
        ...
        # plt.title(f'STRIDE \n MPJPE: {fr_mpjpe:.02f}, PA-MPJPE: {fr_pampjpe:.02f}', fontsize = 20)
        # plt.title(f'PoseformerV2 \n MPJPE: {fr_mpjpe_pf:.02f}, PA-MPJPE: {fr_pampjpe_pf:.02f}', fontsize = 20)
        for i in range(len(joint_pairs)):
            limb = joint_pairs[i]
            # xs, ys, zs = [np.array([j3d[limb[0], j], j3d[limb[1], j]]) for j in range(3)]
            xs1, ys1, zs1 = [
                np.array([j3d1[limb[0], j], j3d1[limb[1], j]]) for j in range(3)
            ]
            # xs2, ys2, zs2 = [np.array([j3d2_hip[limb[0], j], j3d2_hip[limb[1], j]]) for j in range(3)]
            # xs3, ys3, zs3 = [np.array([j3d3_hip[limb[0], j], j3d3_hip[limb[1], j]]) for j in range(3)]
            # if joint_pairs[i] in joint_pairs_left:
            ...
            # ax.plot(-xs2, -zs2, -ys2, color=color_pred, lw=3, marker='o', markerfacecolor='w', markersize=3, markeredgewidth=2, alpha=1.0)
            # ax.plot(-xs3, -zs3, -ys3, color=color_poseformer, lw=3, marker='o', markerfacecolor='w', markersize=3, markeredgewidth=2, alpha=1.0)
