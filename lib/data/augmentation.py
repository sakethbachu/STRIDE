...
class Augmenter2D(object):
    ...
    def add_noise(self, motion_2d):
        ...
        gaussian_sample = (
            torch.randn(batch_size, self.num_Kframes, num_joints, 2) * std + mean
        )
        uniform_sample = (
            torch.rand((batch_size, self.num_Kframes, num_joints, 2)) - 0.5
        ) * uniform_range
        noise_mean = 0
        delta_noise = (
            torch.randn(num_frames, num_joints, 2) * self.noise_std + noise_mean
        )
        # if torch.cuda.is_available():
        ...
        delta_expand = torch.nn.functional.interpolate(
            delta.unsqueeze(1),
        ...
        dis = torch.sqrt(dis2)
        conf = (
            self.dis2conf(dis, a, b, m, s)
            .clip(0, 1)
            .reshape([batch_size, num_frames, num_joints, -1])
        )
        return torch.cat((motion_2d, conf), dim=3)

    def add_3dnoise(self, motion_2d):
        a, b, m, s = (
            self.d2c_params["a"],
            self.d2c_params["b"],
            self.d2c_params["m"],
        ...
        motion_2d_z = motion_2d_z.reshape(batch_size, num_frames, num_joints, -1)
        final_motion_2d = torch.cat((motion_2d, motion_2d_z), dim=3)
        return final_motion_2d
    ...
    def add_mask(self, x):
        ...
        mask_T = (
            torch.rand(1, T, 1, 1, dtype=x.dtype, device=x.device) > self.mask_T_ratio
        )
        x = x * mask * mask_T
        return x
    ...
    def augment2D(self, motion_2d, mask=False, noise=False):
        if noise:
            motion_2d = self.add_3dnoise(motion_2d)
        if mask:
...
class Augmenter3D(object):
    ...
    def augment3D(self, motion_3d):
        # if not self.use_h36m:
        #     self.flip=False
        #     self.scale_range_pretrain = [-1,1]
        if self.scale_range_pretrain:
            # print('check', motion_3d[0])
