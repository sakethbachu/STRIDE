...
def render_overlay_image(
    ...
    scene = pyrender.Scene(bg_color=[0.0, 0.0, 0.0, 0.0], ambient_light=np.ones(3) * 0)
    ...
    camera = pyrender.IntrinsicsCamera(
        fx=focal_length[0], fy=focal_length[1], cx=camera_center[0], cy=camera_center[1]
    )
...
def render_image_group(
    ...
    if keypoints_2d is not None:
        image = draw_skeleton(
            image, kp_2d=keypoints_2d, dataset="spin", unnormalize=False
        )
...
class RendererCam:
    ...
    def visualize_tb(
        ...
        for i in range(nb_max_img):
            rend_img = torch.from_numpy(
                np.transpose(
                    self.__call__(
                        ...
                        images_np[i],
                        vertex_colors=(
                            None if vertex_colors is None else vertex_colors[i]
                        ),
            ...
            if segm_masks is not None:
                mask_img = visualize_segm_masks(
                    images_np[i].copy(), mask=segm_masks[i], alpha=0.8
                )
                        ...
                        side_img = torch.from_numpy(
                            np.transpose(
                                self.__call__(
                                    ...
                                    cam_center=(
                                        None if cam_center is None else cam_center[i]
                                    ),
                    ...
                    side_img = torch.from_numpy(
                        np.transpose(
                            self.__call__(
                                ...
                                joint_labels=(
                                    None if joint_labels is None else joint_labels[i]
                                ),
                                alpha=alpha,
                                sideview=True,
                                camera_rotation=camera_rotation[i],
                                focal_length=(
                                    None if focal_length is None else focal_length[i]
                                ),
                                cam_center=(
                                    None if cam_center is None else cam_center[i]
                                ),
                                mesh_filename=(
                                    mesh_filename.replace(".obj", f"_270_rot.obj")
                                    if mesh_filename
                                    else None
                                ),
                            ),
    ...
    def __call__(
        ...
        material = pyrender.MetallicRoughnessMaterial(
            ...
            alphaMode="OPAQUE",
            baseColorFactor=(
                self.mesh_color[0] / 255.0,
                self.mesh_color[1] / 255.0,
                self.mesh_color[2] / 255.0,
                alpha,
            ),
        )
