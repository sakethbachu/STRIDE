...
def crop_ul_br(img, center, scale, res, rot=0):
    """Crop image according to the supplied bounding box."""
    # Upper left point
...
def generate_heatmaps_2d(
    joints, joints_vis, num_joints=24, heatmap_size=56, image_size=224, sigma=1.75
):
    """
    ...
    joints = (joints + 1.0) * (image_size / 2.0)
...
def generate_part_labels(
    vertices, faces, cam_t, neural_renderer, body_part_texture, K, R, part_bins
):
    ...
    body_parts *= 255.0  # multiply it with 255 to make labels distant
    body_parts, _ = body_parts.max(-1)  # reduce to single channel

    body_parts = torch.bucketize(
        body_parts.detach(), part_bins, right=True
...
def generate_heatmaps_2d_batch(
    joints, num_joints=24, heatmap_size=56, image_size=224, sigma=1.75
):
    ...
    return torch.from_numpy(np.stack(heatmaps)).float().to("cuda"), torch.from_numpy(
        np.stack(heatmaps_vis)
    ).float().to("cuda")
...
def read_exif_data(img_fname):
    ...
    exif = {
        PIL.ExifTags.TAGS[k]: v for k, v in exif_data.items() if k in PIL.ExifTags.TAGS
    }
    return exif
