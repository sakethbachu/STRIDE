...
def show_3d_pose(kp_3d, dataset="common", radius=1, ax=None):
    ...
    for i, (j1, j2) in enumerate(skeleton):
        ...
        for j in range(2):
            if v[j] > 0:  # if visible
                ax.plot(
                    x[j], y[j], z[j], lw=2, c=get_colors()["blue"] / 255, marker="o"
                )
            else:  # nonvisible
...
def draw_skeleton(
    ...
    for idx, pt in enumerate(kp_2d):
        ...
        cv2.circle(image, (pt[0], pt[1]), 3, pcolor, -1)
        ##@@@ to add text and skeleton
        # cv2.putText(image, f'{idx}', (pt[0]+3, pt[1]-3), cv2.FONT_HERSHEY_SIMPLEX, 0.5, pcolor)
    ...
    for pair in joint_pairs:
        limbid1, limbid2 = pair[0], pair[1]
        limb1, limb2 = kp_2d[limbid1], kp_2d[limbid2]
        cv2.line(
            image,
            pt1=(limb1[0], limb1[1]),
            pt2=(limb2[0], limb2[1]),
...
def visualize_segm_masks(image, mask, alpha=0.6, threshold=0.85):
    height, width = image.shape[:2]
    ...
    if len(mask.shape) == 3:
        ...
        # mask = softmax(mask.reshape(mask.shape[0], -1), axis=-1).reshape(*mask_shape)
        # normalize the mask between 0 and 1
        mask = mask.reshape(mask.shape[0], -1)
        mask = (mask - mask.min(1, keepdims=True)) / np.ptp(mask, axis=1, keepdims=True)
        ...
        segm_colors = np.expand_dims(get_segmentation_color_map(), axis=(1, 2))
        mask = mask * segm_colors
        mask = mask.max(0)

    elif len(mask.shape) == 2:
        mask_img = np.zeros((mask.shape[0], mask.shape[1], 3))
        segm_colors = get_segmentation_color_map()

        for part_idx in range(segm_colors.shape[0]):
            indices = np.argwhere(mask == part_idx)
            mask_img[indices[:, 0], indices[:, 1], :] = segm_colors[part_idx]

        mask = mask_img
    else:
        raise ValueError(f"Undefined mask shape {mask.shape}")

    mask = tr.resize(mask, (height, width), anti_aliasing=False)
    mask_img = image * (1 - alpha) + (mask * alpha)
    return mask_img
