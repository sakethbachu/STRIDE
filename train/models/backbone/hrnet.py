    ...
    def forward(self, x):
        ...
        if self.cfg.DOWNSAMPLE:
            if self.cfg.USE_CONV:
                ...
                x1 = F.interpolate(
                    x[0], size=(x0_h, x0_w), mode="bilinear", align_corners=True
                )
                x2 = F.interpolate(
...
def hrnet_w32(
    ...
    cfg = get_cfg_defaults(
        pretrained_ckpt_path, width=32, downsample=downsample, use_conv=use_conv
    )
...
def hrnet_w48(
    ...
    cfg = get_cfg_defaults(
        pretrained_ckpt_path, width=48, downsample=downsample, use_conv=use_conv
    )
    return get_pose_net(cfg, is_train=True)
