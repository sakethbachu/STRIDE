...
from ...core.constants import NUM_JOINTS_SMPL, BN_MOMENTUM
from ...utils.geometry import rot6d_to_rotmat


class HMRHeadCLIFFSMPL(nn.Module):
    def __init__(
        self,
        num_input_features,
        backbone="resnet50",
    ):
        super(HMRHeadCLIFFSMPL, self).__init__()
        npose = NUM_JOINTS_SMPL * 6
        self.npose = npose
        ...
        init_pose = torch.from_numpy(
            mean_params["pose"][: NUM_JOINTS_SMPL * 6]
        ).unsqueeze(0)
    ...
    def forward(
        ...
        pred_rotmat = rot6d_to_rotmat(pred_pose).view(batch_size, NUM_JOINTS_SMPL, 3, 3)
